...
FE는 기본적으로 `http://127.0.0.1:4000`의 BE API와 연동합니다. 앱 시작 시 `GET /health`와 `GET /api/v1/jobs`를 호출하고, 근무 조건 저장 시 `POST /api/v1/jobs` 또는 `PATCH /api/v1/jobs/:jobId`를 사용합니다. 다른 BE 주소를 사용할 때는 `VITE_API_BASE_URL`을 설정합니다.
